...
try {
  ...
  openModal({
    status: "error",
    message: err?.shortMessage || "Withdraw Request Failed",
  });
...
try {
  ...
  openModal({
    status: "success",
    message: "Withdraw Claimed Successfully 🎉",
    hash,
  });
...
try {
  ...
  openModal({
    status: "error",
    message: err?.shortMessage || "Cancel Failed",
  });
...
<div className="p-6 bg-gradient-to-br from-gray-900/80 to-black/80 rounded-2xl border border-white/10 space-y-6 shadow-xl">
  ...
  <div className="space-y-1 text-sm text-gray-400">
    ...
    <div className="text-white">
      Total Available: {formatUnits(totalAvailable, 18)} USDT
    </div>

    <div>
      Monthly Limit: {formatUnits(monthlyLimit, 18)} USDT
    </div>
  ...
  <div className="flex gap-2">
    ...
    <button
      onClick={() =>
        setAmount(formatUnits(totalAvailable, 18))
      }
  ...
  {parsedAmount && (
    ...
    <div className="text-xs text-gray-400 space-y-1">
      ...
      <div>
        Admin Fee (8%): {formatUnits(adminFee, 18)} USDT
      </div>

      <div className="text-green-400">
        You Receive: {formatUnits(userReceive, 18)} USDT
      </div>
  ...
  {pendingWithdraw > 0n && (
    ...
    <div className="p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-xl space-y-3">
      ...
      <div className="flex justify-between text-sm">
        ...
        <span>
          {formatUnits(pendingWithdraw, 18)} USDT
        </span>
      ...
      <div className="flex gap-2">
        ...
        <button
          ...
          className="flex-1 py-2 bg-green-500 text-black rounded-xl disabled:opacity-50"
        >
          Claim Withdraw
        </button>
        ...
        <button
          ...
          className="flex-1 py-2 bg-red-500 text-black rounded-xl disabled:opacity-50"
        >
          Cancel
        </button>
